...
# ============================================
# Auto-start setup (hidden, self-restarting watchdog via Startup folder)
# ============================================
function Setup-Autostart {
    param([string]$mode)

    Write-Host "[INFO] Setting up auto-start for mode: $mode (hidden window, auto-restart on crash)" -ForegroundColor Yellow

    $startupFolder = [Environment]::GetFolderPath('Startup')
    $autostartDir = Join-Path $PROJECT_ROOT '.autostart'
    if (-not (Test-Path $autostartDir)) {
        New-Item -ItemType Directory -Path $autostartDir -Force | Out-Null
    }

    # Clean up old-style visible .bat autostart files from a previous install.ps1 version,
    # so re-running autostart setup doesn't leave a stale visible-window launcher behind
    # alongside the new hidden one.
    foreach ($old in @("VideoController_Server.bat", "VideoController_Agent.bat", "VideoController_Web.bat", "VideoController_Cashier.bat")) {
        $oldPath = Join-Path $startupFolder $old
        if (Test-Path $oldPath) { Remove-Item -Path $oldPath -Force -ErrorAction SilentlyContinue }
    }

    # Find npm (full path baked into the generated scripts - a hidden background
    # process launched at login may not have the same PATH as an interactive shell)
    $npmPath = $null
    try {
    ...
    if (-not $npmPath) {
        $foundNode = Find-NodeJS
        if ($foundNode) {
            $candidate = Join-Path (Split-Path $foundNode -Parent) 'npm.cmd'
            if (Test-Path $candidate) { $npmPath = $candidate }
        }
    }
    if (-not $npmPath) {
        Write-Host "[ERROR] npm tidak ditemukan, tidak bisa setup auto-start" -ForegroundColor Red
        return
    }

    $startedVbs = @()

    # Creates a hidden PowerShell watchdog (keeps $ServiceName running, restarts it if it
    # crashes) plus a Startup-folder .vbs launcher that starts that watchdog with no
    # visible window at all (WScript.Shell.Run's window-style 0 = hidden).
    # NOTE: this only hides the terminal/console window - if $ServiceName is the agent,
    # the actual Chrome browser it drives is a separate process and still shows normally.
    function New-ServiceWatchdog {
        param(
            [string]$ServiceName,
            [string]$WorkDir,
            [string]$NpmArgs,
        ...
        $logFile = Join-Path $PROJECT_ROOT "$($ServiceName.ToLower())_startup.log"
        $errFile = Join-Path $PROJECT_ROOT "$($ServiceName.ToLower())_startup.err.log"
        $watchdogPidFile = Join-Path $autostartDir "$ServiceName.watchdog.pid"
        $procPidFile = Join-Path $autostartDir "$ServiceName.process.pid"
        $watchdogScript = Join-Path $autostartDir "VideoController_$ServiceName.ps1"
        $vbsLauncher = Join-Path $startupFolder "VideoController_$ServiceName.vbs"

        $watchdogContent = @"
# Auto-generated by install.ps1 - do not edit by hand, re-run install.ps1 autostart to regenerate.
# Keeps $ServiceName running hidden in the background and restarts it if it exits/crashes.
`$PID | Out-File -FilePath '$watchdogPidFile' -Force
$ExtraEnvLine
Set-Location '$WorkDir'
while (`$true) {
    "`$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - Starting $ServiceName..." | Out-File -FilePath '$logFile' -Append
    `$p = Start-Process -FilePath '$npmPath' -ArgumentList '$NpmArgs' -WorkingDirectory '$WorkDir' -WindowStyle Hidden -RedirectStandardOutput '$logFile' -RedirectStandardError '$errFile' -PassThru
    `$p.Id | Out-File -FilePath '$procPidFile' -Force
    Wait-Process -Id `$p.Id -ErrorAction SilentlyContinue
    "`$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') - $ServiceName exited (code `$(`$p.ExitCode)), restarting in 5s..." | Out-File -FilePath '$logFile' -Append
...
objShell.Run "powershell.exe -NoProfile -ExecutionPolicy Bypass -WindowStyle Hidden -File ""$watchdogScript""", 0, False
"@
        Set-Content -Path $vbsLauncher -Value $vbsContent -Encoding ASCII

        Write-Host "[OK] $ServiceName auto-start configured (hidden, auto-restart)" -ForegroundColor Green
    ...
    if ($mode -eq "room" -or $mode -eq "all") {
        $startedVbs += New-ServiceWatchdog -ServiceName "Server" -WorkDir (Join-Path $PROJECT_ROOT 'server') -NpmArgs "run start"
        $startedVbs += New-ServiceWatchdog -ServiceName "Agent" -WorkDir (Join-Path $PROJECT_ROOT 'agent') -NpmArgs "run start" -ExtraEnvLine "`$env:BROWSER_HEADLESS = 'false'"
        $startedVbs += New-ServiceWatchdog -ServiceName "Web" -WorkDir (Join-Path $PROJECT_ROOT 'web') -NpmArgs "run preview:host"
    }

    if ($mode -eq "kasir" -or $mode -eq "all") {
        $startedVbs += New-ServiceWatchdog -ServiceName "Cashier" -WorkDir (Join-Path $PROJECT_ROOT 'cashier') -NpmArgs "run preview:host"
    }

    Write-Host ""
    Write-Host "[INFO] Auto-start jalan pas login, TANPA jendela CMD yang muncul." -ForegroundColor Yellow
    Write-Host "[INFO] Kalau service-nya crash, otomatis di-restart sendiri (nunggu 5 detik)." -ForegroundColor Yellow
    Write-Host "[INFO] Browser Chrome (buat video di agent) TETEP muncul normal - cuma terminal-nya yang disembunyiin." -ForegroundColor Yellow
    Write-Host "[INFO] Cara cek jalan apa nggak: buka file *_startup.log di $PROJECT_ROOT" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "[Q] Jalanin sekarang juga (gak perlu logout/login dulu)? [y/N]: " -ForegroundColor Cyan -NoNewline
    $runNow = Read-Host
    if ($runNow -eq 'y' -or $runNow -eq 'Y') {
        foreach ($vbs in $startedVbs) {
            Write-Host "[INFO] Starting $(Split-Path $vbs -Leaf)..." -ForegroundColor Yellow
...
function Remove-Autostart {
    ...
    $servicesToRemove = @()
    if ($mode -eq "room" -or $mode -eq "all") { $servicesToRemove += @("Server", "Agent", "Web") }
    if ($mode -eq "kasir" -or $mode -eq "all") { $servicesToRemove += @("Cashier") }

    foreach ($svc in $servicesToRemove) {
        $watchdogPidFile = Join-Path $autostartDir "$svc.watchdog.pid"
        $procPidFile = Join-Path $autostartDir "$svc.process.pid"

        # Stop the watchdog first so it doesn't immediately relaunch what we're about to kill
        ...
        # Stop the running npm/node process tree (taskkill /T also stops child processes,
        # e.g. the node process npm spawns - Stop-Process alone would leave it running)
        if (Test-Path $procPidFile) {
            $ppid = Get-Content $procPidFile -ErrorAction SilentlyContinue
            if ($ppid) {
                & taskkill.exe /PID $ppid /T /F 2>$null | Out-Null
            }
            Remove-Item $procPidFile -Force -ErrorAction SilentlyContinue
        }

        # Remove the Startup-folder launcher and generated watchdog script
        Remove-Item (Join-Path $startupFolder "VideoController_$svc.vbs") -Force -ErrorAction SilentlyContinue
        Remove-Item (Join-Path $autostartDir "VideoController_$svc.ps1") -Force -ErrorAction SilentlyContinue

        # Clean up any leftover old-style .bat from a previous install.ps1 version
        Remove-Item (Join-Path $startupFolder "VideoController_$svc.bat") -Force -ErrorAction SilentlyContinue

        Write-Host "[OK] $svc auto-start removed (process stopped, launcher deleted)" -ForegroundColor Green
    }

    Write-Host "[OK] Auto-start ($mode) removed successfully!" -ForegroundColor Green
    Write-Host "[INFO] Kalau browser Chrome dari agent masih kebuka, tutup manual - itu proses terpisah dari watchdog." -ForegroundColor Yellow
}
...
if ($INSTALL_MODE -eq 'all' -or $INSTALL_MODE -eq 'room') {
    ...
    # Start Server first
    $serverProcess = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'npm run start') -WorkingDirectory (Join-Path $PROJECT_ROOT 'server') -WindowStyle Hidden -RedirectStandardOutput (Join-Path $PROJECT_ROOT 'server_startup.log') -RedirectStandardError (Join-Path $PROJECT_ROOT 'server_startup.err.log') -PassThru
    ...
    if (-not $serverProcess.HasExited) {
        $agentProcess = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'set BROWSER_HEADLESS=false&& npm run start') -WorkingDirectory (Join-Path $PROJECT_ROOT 'agent') -WindowStyle Hidden -RedirectStandardOutput (Join-Path $PROJECT_ROOT 'agent_startup.log') -RedirectStandardError (Join-Path $PROJECT_ROOT 'agent_startup.err.log') -PassThru
    ...
    # Start Web
    $webProcess = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'npm run preview:host') -WorkingDirectory (Join-Path $PROJECT_ROOT 'web') -WindowStyle Hidden -RedirectStandardOutput (Join-Path $PROJECT_ROOT 'web_startup.log') -RedirectStandardError (Join-Path $PROJECT_ROOT 'web_startup.err.log') -PassThru
...
if ($INSTALL_MODE -eq 'all' -or $INSTALL_MODE -eq 'kasir') {
    ...
    $cashierProcess = Start-Process -FilePath 'cmd.exe' -ArgumentList @('/c', 'npm run preview:host') -WorkingDirectory (Join-Path $PROJECT_ROOT 'cashier') -WindowStyle Hidden -RedirectStandardOutput (Join-Path $PROJECT_ROOT 'cashier_startup.log') -RedirectStandardError (Join-Path $PROJECT_ROOT 'cashier_startup.err.log') -PassThru
    $processes += $cashierProcess
    Write-Host "   - Cashier: PID $($cashierProcess.Id)" -ForegroundColor Cyan
}

Write-Host ""
Write-Host "[OK] All selected services started! (jendela terminal disembunyiin - cek *_startup.log di $PROJECT_ROOT kalau perlu debug)" -ForegroundColor Green
Write-Host ""
